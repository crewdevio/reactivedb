...
/**
 * implementation of AES ( Advanced Encryption Standard ) in Javascript
 */
export class AES {
  static #sBox = [
    0x63,
  ...
  static #utf8Encode(text) {
    return unescape(encodeURIComponent(text));
  }

  static #utf8Decode(text) {
    try {
  ...
  static get Ctr() {
    return {
       ...
       * @param {"128" | "192" | "256"} nBits - bits size
       * ```ts
       ...
       * ```
       */
      decrypt(ciphertext, password, nBits) {
       ...
       * ```
       */
      encrypt(plaintext, password, nBits) {
        const blockSize = 16; // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
        if (
          !(
            parseInt(nBits) === 128 ||
        ...
        for (let b = 0; b < blockCount; b++) {
          ...
          const blockLength = b < blockCount - 1
            ? blockSize
