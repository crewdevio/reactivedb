...
import { FieldFilter, MapperKind, RootFieldFilter } from "./Interfaces.ts";
...
export function filterSchema({
  ...
  const filteredSchema: any = mapSchema(schema, {
    [MapperKind.QUERY]: (type: any) =>
      filterRootFields(type, "Query", rootFieldFilter),
    [MapperKind.MUTATION]: (type: any) =>
      filterRootFields(type, "Mutation", rootFieldFilter),
    [MapperKind.SUBSCRIPTION]: (type: any) =>
      filterRootFields(type, "Subscription", rootFieldFilter),
    [MapperKind.OBJECT_TYPE]: (type: any) =>
      typeFilter((type as any).name, type)
        ? filterObjectFields(type, fieldFilter as any)
        : null,
    [MapperKind.INTERFACE_TYPE]: (
      type: any,
    ) => (typeFilter((type as any).name, type) ? undefined : null),
